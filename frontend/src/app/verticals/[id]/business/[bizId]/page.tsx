...
import { ChevronLeft, ChevronRight, DollarSign, RefreshCw, Plus, X, Users } from "lucide-react";
...
export default function BusinessDetailPage() {
  ...
  const applyDates = () => load(granularity, startDate, endDate);
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-0.5">
              {business?.owner_name ? (
                <span className="text-xs text-black">Owner: <span className="font-semibold">{business.owner_name}</span>
                  {business.owner_email && <span className="text-gray-500"> · {business.owner_email}</span>}
                </span>
              ) : (
                <span className="text-xs text-gray-400">No owner assigned</span>
              )}
              <button onClick={() => { setOwnerName(business?.owner_name || ""); setOwnerEmail(business?.owner_email || ""); setShowAddOwner(true); }}
                className="text-xs font-bold text-blue-900 hover:underline flex items-center gap-1">
                <Users className="w-3 h-3" /> {business?.owner_name ? "Edit Owner" : "Add Owner"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          <button onClick={applyDates} className="px-3 py-2 bg-blue-900 hover:bg-blue-800 text-white text-xs font-bold rounded-md transition">
            Apply
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-lg border border-gray-300 shadow-sm p-5">
              <div className="text-[10px] font-bold uppercase tracking-wider text-black mb-2">Resources</div>
              <div className="text-2xl font-bold text-blue-900">{costData?.resource_count || 0}</div>
              <div className="text-xs text-black mt-1">tagged resources</div>
            </div>
            <div className="bg-white rounded-lg border border-gray-300 shadow-sm p-5">
              <div className="text-[10px] font-bold uppercase tracking-wider text-black mb-2">Business</div>
              <div className="text-base font-bold text-black">{business?.name}</div>
              <div className="text-xs text-black mt-1" style={{ color: vertical?.color }}>{vertical?.name} vertical</div>
            </div>
          </div>
          ...
          {trend.length === 0 && (
            <div className="bg-white rounded-lg border border-gray-300 shadow-sm p-12 text-center mb-6">
              <p className="text-sm font-bold text-black">No cost data for selected period</p>
              <p className="text-xs text-gray-500 mt-1">Tag resources with Business={business?.name} to see cost here</p>
            </div>
          )}
        </>
      )}
