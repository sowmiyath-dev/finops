...
export default function VerticalsPage() {
  ...
  const fetchVerticals = async () => {
    const res = await axios.get(`${API}/api/verticals/`, { headers });
    return res.data as VerticalItem[];
  };

  const fetchCost = async (id: string, gran: string) => {
    try {
      const res = await axios.get(`${API}/api/verticals/${id}/cost`, {
        headers,
        params: { granularity: gran },
      });
      return res.data as VerticalCost;
    } catch {
      return null;
    }
  };
  ...
  const load = async (gran = granularity) => {
    setLoading(true);
    try {
      const verts = await fetchVerticals();
      setVerticals(verts);
      const costs: Record<string, VerticalCost> = {};
      await Promise.all(
        verts.map(async (v) => {
          const c = await fetchCost(v.id, gran);
          if (c) costs[v.id] = c;
        })
      );
      setCostMap(costs);
  ...
  const seed = async () => {
    setSeeding(true);
    try {
      await axios.post(`${API}/api/verticals/seed`, {}, { headers });
      await load();
    } finally {
      setSeeding(false);
    }
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-black mt-1">
            Business unit cost visibility across all cloud providers
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg border border-gray-300 p-12 text-center">
          ...
          <p className="text-xs text-black mb-4">Click "Seed Verticals" to create the default 4 verticals</p>
        </div>
      ) : (
          ...
          {verticals.map((v) => {
            ...
            return (
              ...
              >
                {/* Card header */}
                <div
                  className="rounded-t-lg px-5 py-4 flex items-center justify-between"
                ...
                <div className="grid grid-cols-3 divide-x divide-gray-200 border-b border-gray-200">
                  {[
                    { icon: DollarSign, label: "Total Cost", value: `$${total.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
                    { icon: Users, label: "Owners", value: ownerCount(v) },
                    { icon: Box, label: "Applications", value: appCount(v) },
                  ].map((stat) => (
