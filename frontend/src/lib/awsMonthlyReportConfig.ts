// Fixed application mapping config — account IDs are the source of truth, names are display only
// Edit this file to update application groupings

export interface AppMapping {
  appName: string;
  note: string;
  // Each entry: { accountId, fraction } — fraction defaults to 1 (full cost)
  accounts: { accountId: string; fraction?: number }[];
}

// ── Novac CT account IDs ──────────────────────────────────────────────────────
export const NOVAC_SHARED_SERVICES_ID = "240329355338";
export const NOVAC_PAYER_ID           = "10241470425";  // Redington limited — excluded from shared split

// SFL group — SFL-SHARED-SERVICE splits into SFL-PROD + SFL-UAT only
export const SFL_SHARED_ID  = "833660969797";
export const SFL_PROD_ID    = "683092765314";
export const SFL_UAT_ID     = "400487655910";

// ── WonderlendHubs & CreditNirvana CT ────────────────────────────────────────
export const WH_CT_ACCOUNT_ID        = "339712884147"; // NOVACwonderlendhubs
export const CN_ACCOUNT_IDS          = ["730335499592", "890742572706", "471112760393"]; // NOVACcreditnirvana + UAT + Redington

// ── Automall CT account IDs ───────────────────────────────────────────────────
export const AUTOMALL_PAYER_ID = "431309843953"; // Redington limited (Payer)

// ── Indostar CT account IDs ───────────────────────────────────────────────────
export const INDOSTAR_PAYER_ID = "312255911599"; // Redington Ltd (Payer)

// ── Application master mapping ────────────────────────────────────────────────
// For CT-level entries (Automall, Indostar, WH, CN) accountId is set to the CT id marker
// and resolved at runtime from CT total cost
export const APP_MAPPINGS: AppMapping[] = [
  { appName: "Finergy",              note: "Novac-EBS/2",                                          accounts: [{ accountId: "583617605382", fraction: 0.5 }] },
  { appName: "Pahal",                note: "Novac-EBS/2",                                          accounts: [{ accountId: "583617605382", fraction: 0.5 }] },
  ...
  { appName: "Rnd",                  note: "Novac-RnD",                                            accounts: [{ accountId: "78418245182" }] },
  ...
  { appName: "Novac Credit Nirvana", note: "NOVACcreditnirvana+NOVACcreditnirvana UAT+Redington(471112760393)", accounts: CN_ACCOUNT_IDS.map((id) => ({ accountId: id })) },
];
